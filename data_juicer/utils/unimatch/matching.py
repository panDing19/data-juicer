...
def local_correlation_softmax(
    ...
    # Ensure tensors are on the correct device and are float32
    # This might be redundant if already handled, but good to ensure
    feature1 = feature1.to(torch.float32).cuda() # Or whatever device your model expects
    sample_coords_norm = sample_coords_norm.to(torch.float32).cuda()

    # Explicitly make contiguous again, just to be absolutely sure
    feature1_c = feature1.contiguous()
    sample_coords_norm_c = sample_coords_norm.contiguous()


    # Check range of grid values if align_corners=True, they should be in [-1, 1]
    sample_coords_norm_c = torch.clamp(sample_coords_norm_c, min=-0.99, max=0.99)

    window_feature = F.grid_sample(
        feature1_c,
        sample_coords_norm_c,
        padding_mode=padding_mode,
        align_corners=True
    ).permute(0, 2, 1, 3)  # [B, H*W, C, (2R+1)^2]

    # sample_coords_norm = torch.clamp(sample_coords_norm, min=-1.0, max=1.0)
    # window_feature = F.grid_sample(feature1.contiguous(), sample_coords_norm.contiguous(), padding_mode=padding_mode, align_corners=True).permute(
    #     0, 2, 1, 3
    # )  # [B, H*W, C, (2R+1)^2]
    ...
    min_val = torch.finfo(corr.dtype).min
    corr[~valid] = min_val
...
def local_correlation_softmax_stereo(
    ...
    # normalize coordinates to [-1, 1]
    sample_coords_norm = normalize_coords(sample_coords, h, w)  # [-1, 1]
    window_feature = F.grid_sample(feature1.contiguous(), sample_coords_norm.contiguous(), padding_mode="zeros", align_corners=True).permute(
        0, 2, 1, 3
    )  # [B, H*W, C, (2R+1)]
...
def warp_with_pose_depth_candidates(
    ...
    warped_feature = F.grid_sample(
        feature1.contiguous(), grid.view(b, d * h, w, 2).contiguous(), mode="bilinear", padding_mode="zeros", align_corners=True
    ).view(
        b, c, d, h, w
    )  # [B, C, D, H, W]
